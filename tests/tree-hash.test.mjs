// tree-hash.py, the vendored copy's integrity proof.
//
// It shipped IMPORTING A MODULE THAT WAS NEVER COMMITTED, so every invocation raised
// ModuleNotFoundError. Nothing noticed: sync-skill.sh computes the digest as
// `python3 tree-hash.py … 2>/dev/null || echo unavailable`, so the pin recorded
// `tree_sha256=unavailable` and the check the pin's own header documents was inert in every
// consumer at once. A test that merely RUNS the script is what was missing — these rows exist
// before any of the behavioural ones for that reason.
