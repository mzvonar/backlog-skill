...
//
//   node validate.mjs <artifacts-dir>
//
...
/**
 * @param {string} root the artifacts directory holding deferred-work.md and deferred-work/
 * @returns {{code: string, where: string, detail: string}[]} findings, empty when clean
 */
export const scanBacklog = (root) => {
  const findings = [];
  const add = (code, where, detail) => findings.push({ code, detail, where });

  const index = path.join(root, "deferred-work.md");
  const dir = path.join(root, "deferred-work");
  if (!existsSync(index)) {
    add("NO_INDEX", "deferred-work.md", "the index does not exist");
    return findings;
  }
  ...
  if (existsSync(dir) && statSync(dir).isDirectory()) {
    for (const name of readdirSync(dir).toSorted()) {
      if (!name.endsWith(".md")) {
        add("STRAY_FILE", `deferred-work/${name}`, "not a .md detail file");
        continue;
      }
      const fm = frontmatter(readFileSync(path.join(dir, name), "utf-8"));
      if (fm === null) {
        add("NO_FRONTMATTER", `deferred-work/${name}`, "no `---` frontmatter block — nothing can classify it");
        continue;
      }
      for (const key of REQUIRED) {
        if (!fm[key]) {
          add("MISSING_FIELD", `deferred-work/${name}`, `frontmatter has no \`${key}\``);
        }
      }
      if (fm.id && seen.has(fm.id)) {
        add("DUPLICATE_ID", `deferred-work/${name}`, `id \`${fm.id}\` is also used by ${seen.get(fm.id)}`);
      }
      if (fm.id) {
        seen.set(fm.id, name);
      }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!root) {
    console.error("usage: validate.mjs <artifacts-dir>");
    process.exit(2);
  }
  let findings;
  try {
    findings = scanBacklog(root);
  } catch (error) {
