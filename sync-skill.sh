#!/usr/bin/env bash
# sync-skill.sh — keep the backlog skill in sync between this repo and the repos that use it.
#
# Forward (this repo → consumer), the normal flow. A COMMITTED copy lands in
# <consumer>/.claude/skills/backlog plus <consumer>/.claude/skills/.backlog-version:
...
#   ./sync-skill.sh --from <consumer-repo>          # copies consumer's .claude/skills/backlog
#                                                   # over skills/backlog here; review + commit here,
...
if [ -n "$FROM" ]; then
  ...
  PIN="$FROM/.claude/skills/.$SKILL-version"
  if [ -f "$PIN" ]; then
...
# Pull edits made here back upstream:  <backlog-skill>/sync-skill.sh --from <this-repo>
...
echo "Wrote $DEST/.$SKILL-version"
echo "Next in $CONSUMER: review the diff, then commit .claude/skills/$SKILL + .claude/skills/.$SKILL-version."
